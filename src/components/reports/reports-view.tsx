    ...
    <div className="flex flex-col h-full bg-bg-0">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-line">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-fg-0">生产报工</h1>
      ...
      {/* 统计 */}
      <div className="grid grid-cols-3 gap-3 px-6 py-3 border-b border-line">
        <div className="bg-bg-1 border border-line rounded p-3">
          <div className="text-xs text-fg-2">报工批次总数</div>
          <div className="text-2xl font-mono tabular-nums text-fg-0 mt-1">{stats.total}</div>
        </div>
        <div className="bg-bg-1 border border-line rounded p-3">
          <div className="text-xs text-fg-2">进行中</div>
        ...
        <div className="bg-bg-1 border border-line rounded p-3">
          <div className="text-xs text-fg-2">已关闭</div>
      ...
      <div className="flex items-center gap-3 px-6 py-3 border-b border-line">
        <input
          ...
          className="flex-1 bg-bg-1 border border-line rounded px-3 py-1.5 text-sm text-fg-0 placeholder:text-fg-3 focus:outline-none focus:border-orange-500/50"
        />
      </div>
        ...
        {loading ? (
          <div className="space-y-2">
            {Array.from({ length: 6 }).map((_, i) => (
              <Skeleton key={i} className="h-12 w-full bg-bg-1" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-fg-2">
            <FileText className="w-10 h-10 mb-2 text-fg-3" />
          ...
          <div className="bg-bg-1 border border-line rounded overflow-hidden">
            <div className="grid grid-cols-[100px_140px_120px_120px_160px_140px_120px_80px_100px_40px] gap-0 bg-bg-2 border-b border-line text-xs text-fg-1 font-medium">
            ...
            {filtered.map((r) => (
              <div
                key={r.id}
                className="grid grid-cols-[100px_140px_120px_120px_160px_140px_120px_80px_100px_40px] gap-0 border-b border-line/60 hover:bg-bg-3 text-sm items-center"
              >
                <div className="px-3 py-2.5 font-mono text-xs text-fg-1 truncate" title={r.report_no}>
                  {r.report_no}
                </div>
                <div className="px-3 py-2.5 font-mono text-fg-0 truncate">{r.work_order_no}</div>
                <div className="px-3 py-2.5 font-mono text-fg-0">#{r.completion_seq}</div>
                <div className="px-3 py-2.5 font-mono text-fg-0">{r.batch_no}</div>
                <div className="px-3 py-2.5 font-mono text-xs text-fg-1">
                  {r.start_time ? formatDateTime(r.start_time) : "—"}
                </div>
                <div className="px-3 py-2.5 font-mono text-xs text-fg-1">
                  {r.end_time ? formatDateTime(r.end_time) : "—"}
                </div>
                <div className="px-3 py-2.5 font-mono tabular-nums text-fg-0 text-right text-xs">
                  {r.input_quantity} / {r.pass_quantity} / {r.fail_quantity}
                </div>
                <div className="px-3 py-2.5 font-mono text-xs text-fg-1 text-center">
                ...
                <div className="px-3 py-2.5 flex justify-end">
                  <button
                    ...
                    className="text-fg-2 hover:text-orange-500 transition-colors"
                    title="查看工单"
                  >
